...
void multiple_commit_test(void)
{
    struct commit* commit_list = NULL;
    int retval;
    retval = beargit_init();
...
int cunittester()
{
   ...
   if (NULL == CU_add_test(pSuite2, "Simple commit output test", simple_commit_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
   ...
   /* Add tests to the Suite #2 */
   if (NULL == CU_add_test(pSuite3, "Multi-commit output test", multiple_commit_test))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
